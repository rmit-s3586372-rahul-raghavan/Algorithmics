...
public class DoubleLinkedList implements MyList
{
    ...
    public void add(int index, int newValue) throws IndexOutOfBoundsException {
        ...
        // YOUR IMPLEMENTATION
        Node newNode = new Node(newValue);
        int middle = mLength/2;

	// Perform a forward traversal if the index is less than or equal to half the length
	if(index <= middle) {
                if( index == 0) {
			newNode.setNext(mHead);
			mHead = newNode;
	...
	// Perform a reverse traversal if the index is greater than the length
	else {
		if(index == mLength-1) {
    ...
    public boolean remove(int value) {
		...
		if (currNode.getValue() == value) {
			if (currNode == mHead) {
				Node newNode = currNode.getNext();
				mHead = newNode;
				newNode.setPrev(null);		
			}
			else if (currNode == mTail) {
				Node newNode = currNode.getPrev();
				mTail = newNode;
